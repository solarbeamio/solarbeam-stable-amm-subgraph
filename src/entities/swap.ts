...
import { SwapFlashLoanNoWithdrawFee } from "../../generated/3pool/SwapFlashLoanNoWithdrawFee"
import { getOrCreateToken } from "./token"
import { getSystemInfo } from "./system"
import {getOrCreateTokenIndex} from "./tokenIndex";
...
export function getOrCreateSwapNoWithdrawFee(
  ...
  if (swap == null) {
    ...
    swap.tokenIndexes = info.tokenIndexes
    swap.A = info.A
...
export function getSwapInfoNoWithdrawFee(swap: Address): SwapInfo {
  ...
  do {
    ...
    if (!t.reverted && t.value.toHexString() != ZERO_ADDRESS) {
      tokens.push(t.value)
      let tokenIndex = getOrCreateTokenIndex(BigInt.fromI32(i), t.value)
      tokenIndexes.push(tokenIndex.id)
    }
